...
class guestregistration(models.Model):
    ...
    guestname=fields.Char("Guest Name",related='guest_id.name', readonly=False)
    ...
    def action_reserve(self):
        for rec in self:
            if not(rec.guest_id):  
                ...
                pkid = rec.id
                # Call PostgreSQL function to check schedule conflict
                self.env.cr.execute("SELECT * FROM public.fnCheck_RegistrationConflict(%s)", (pkid,))
